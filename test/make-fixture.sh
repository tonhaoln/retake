#!/usr/bin/env bash
# Regenerates test/fixture.osrec from scratch (ffmpeg + python3 required).
# VP9/Opus on purpose: Playwright's Chromium lacks H.264/AAC; the editor has fallbacks.
set -euo pipefail
cd "$(dirname "$0")"
mkdir -p fixture.osrec
ffmpeg -y -loglevel error -f lavfi -i "testsrc2=size=1280x800:rate=30:duration=5,drawbox=x=380:y=280:w=200:h=120:color=white@0.8:t=6" \
  ...
  -pix_fmt yuv420p -c:v libvpx-vp9 -b:v 2M -c:a libopus -shortest fixture.osrec/screen.mp4
ffmpeg -y -loglevel error -f lavfi -i "smptebars=size=640x480:rate=30:duration=5" \
  -f lavfi -i "sine=frequency=440:duration=5" \
  -pix_fmt yuv420p -c:v libvpx-vp9 -c:a libopus -shortest fixture.osrec/webcam.webm
python3 - <<'PY'
import json, math
samples = []
...
json.dump({"samples":samples,"clicks":clicks,"keys":[2.5,2.6,2.7],
           ...
           "clicksCaptured":True}, open("fixture.osrec/cursor.json","w"))
json.dump({"version":2,"screen":"screen.mp4","cursor":"cursor.json","webcam":"webcam.webm",
           ...
           "capture":{"mode":"display","index":0}}, open("fixture.osrec/meta.json","w"))
