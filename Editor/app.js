...
async function loadFromFiles(fileList) {
  ...
  if (files[camName]) {
    ...
    if (!ok) { toast('Webcam file could not be decoded — continuing without it.'); webcam = null; }
...
function autoZoomFromClicks(quiet = false) {
  ...
  for (const c of downs) {
    const x = c.x * S.pointScale, y = c.y * S.pointScale;
...
$('cropReset').onclick = () => { if (S.set.crop) pushUndo(); S.set.crop = null; exitCropMode(false); };
...
let aimDrag = false, cropDrag = null;
...
preview.addEventListener('pointercancel', () => { aimDrag = false; cropDrag = null; });
preview.addEventListener('lostpointercapture', () => { aimDrag = false; cropDrag = null; });
...
preview.addEventListener('pointerup', () => { aimDrag = false; cropDrag = null; });
function movAim(e) {
  ...
  // invert content mapping at current camera
  const cr = cropRect();
  ...
  s.x = clamp(sx + (mx - rx) / rw * sw, cr.x, cr.x + cr.w);
  s.y = clamp(sy + (my - ry) / rh * sh, cr.y, cr.y + cr.h);
  requestRender();
}
